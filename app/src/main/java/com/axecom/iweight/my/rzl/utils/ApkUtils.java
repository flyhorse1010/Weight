...
public class ApkUtils implements  VolleyListener{
    private static final String TAG = com.axecom.iweight.my.rzl.utils.ApkUtils.class.getSimpleName();
...
//            }
    ...
    public static void installUseAS(Context context,String filePath) {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                ...
                Uri contentUri = FileProvider.getUriForFile(com.axecom.iweight.my.rzl.utils.ApkUtils.getInstance().ctx, "com.axecom.iweight.fileprovider", apkFile);//com.dtncpzs.traceback.fileprovider
    ...
    public static com.axecom.iweight.my.rzl.utils.ApkUtils getInstance(){
        if(myself==null){
            myself=new com.axecom.iweight.my.rzl.utils.ApkUtils();
        }
        return myself;
    }
}
